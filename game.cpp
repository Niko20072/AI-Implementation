...
namespace Tmpl8
{
    ...
    {
    public:
        Tank(float ix, float iy, float ispeed = 2.5f, int idirection = 0)
        {
            x = ix;
            y = iy;
    ...
    {
    public:
        AITank(float ix, float iy, int idirection = 0) : Tank(ix, iy, 1.5f, idirection) //Make sure the AI Tank is slower than the player 
        {
        }
        void Move(Tank* target)
        {
			//target -> x and y are the player tank coordinates
            if (target->x < 500 && target->x > 100 && target->y < 450 && target->y > 150)
            {
                float angle = ((2 * PI) / 16) * direction;
                x += (sinf(angle) * speed);
                y += (-cosf(angle) * speed);
